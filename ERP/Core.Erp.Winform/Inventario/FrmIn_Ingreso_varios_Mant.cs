...
namespace Core.Erp.Winform.Inventario
{
    public partial class FrmIn_Ingreso_varios_Mant : Form
    {
        ...
        int IdProducto_tipo_lote = 0;
        string mensajeErrorPantalla = string.Empty;
        ...
        in_Producto_Info i_producto_padre = new in_Producto_Info();
       //Infos
...
//        in_Ing_Egr_Inven_Info info_IngEgr = new in_Ing_Egr_Inven_Info();
                ...
                else
                {
                    switch (param.IdCliente_Ven_x_Default)
                    {
                        case Cl_Enumeradores.eCliente_Vzen.FJ:
                            ...
                            int IdSucursal = Convert.ToInt32(ucIn_Sucursal_Bodega1.cmb_sucursal.EditValue);
                            int IdBodega = Convert.ToInt32(ucIn_Sucursal_Bodega1.cmb_bodega.EditValue);
                            decimal IdNumMovi = Convert.ToDecimal(txtNumIngreso.Text);
                            int IdMovi_inven_tipo = Convert.ToInt32(cmbTipoMovi.get_TipoMoviInvInfo().IdMovi_inven_tipo);
        ...
        void carga_Combos_Productos_y_Tipo_Movi()
        {
            try
            {
                ...
                if (ucIn_Sucursal_Bodega1.cmb_bodega.EditValue!=null)
                {
                    ...
                    cmbProducto_grid.DisplayMember = "pr_descripcion_combo";
        ...
        void carga_Combos()
        {
            try
            {
                ...
                in_Parametro_Info info_parametros = bus_param_in.Get_Info_Parametro(param.IdEmpresa);
                IdProducto_tipo_lote = Convert.ToInt32(info_parametros.P_IdProductoTipo_para_lote_0);
                carga_Combos_Productos_y_Tipo_Movi();

                cmbTipoMovi.readonly_combo(true);
                cmbTipoMovi.cargar_TipoMotivo(0, 0, "", "");
                if (info_parametros.P_IdMovi_inven_tipo_default_ing != null)
                    cmbTipoMovi.set_TipoMoviInvInfo(Convert.ToInt32(info_parametros.P_IdMovi_inven_tipo_default_ing));
...
list_unidad_medida = Bus_Uni_Med.Get_list_UnidadMedida();
        ...
        void Inhabilita_Controles()
        {
            try
            {
                ...
                if (Info_ing_egr_Inven_.IdEstadoAproba == "APRO")
                {
        ...
        private void LimpiarDatos()
        {
            try
            {
                ...
                gridViewProductos.OptionsBehavior.ReadOnly = false;
        ...
        #region set get info
                ...
                void Get_Info()
                {
                    try
                    {
                        ...
                        Info_ing_egr_Inven_.listIng_Egr.ForEach(q => { q.IdBodega = Info_ing_egr_Inven_.IdBodega; });
                    }
                    catch (Exception ex)
                    {
                        Log_Error_bus.Log_Error(ex.ToString());
            ...
            try
            {
                ...
                switch (Accion)
                {
                    case Cl_Enumeradores.eTipo_action.grabar:
                        ...
                        if(Bus_IngEgr.GuardarDB_aprobar_contabilizar(Info_ing_egr_Inven_))
                        {
                            txtNumIngreso.Text = Convert.ToString(Info_ing_egr_Inven_.IdNumMovi);
                    ...
                    case Cl_Enumeradores.eTipo_action.actualizar:
                        ...
                        if (Bus_IngEgr.ModificarDB_aprobar_contabilizar(Info_ing_egr_Inven_))
                        {
        ...
        private Boolean Agregar_fila_copiada(string data)
        {
            try
            {
                ...
                //codigo produc | observacion | unidad medida |cant |costo| codigoLote | FechaFab | FechaVcto
                ...
                if (rowData.Count() >= 3) //return false;          
                {
                    ...
                    in_Producto_Info QProducto;
                    if (!string.IsNullOrEmpty(rowData[5]))
                        QProducto = listProducto.FirstOrDefault(v => v.IdEmpresa == param.IdEmpresa && v.pr_codigo == cod_producto && v.lote_num_lote == rowData[5].ToString());
                    else
                        QProducto = listProducto.FirstOrDefault(v => v.IdEmpresa == param.IdEmpresa && v.pr_codigo == cod_producto);
                    if (QProducto != null)
                    { Info_Producto = QProducto; }
                    ...
                    double costo_unitario = rowData[4] == "" ? 0 : Convert.ToDouble(rowData[4]);
                    string observacion = Convert.ToString(rowData[1]);

                    if (!string.IsNullOrEmpty(rowData[5]))
                    {
                        string codigo_lote = Convert.ToString(rowData[5]);

                        decimal ID_prod_lote = Bus_Producto.get_id_x_lote(param.IdEmpresa, cod_producto, codigo_lote);
                        if (ID_prod_lote == 0)
                        {

                            DateTime? FechaFab = null;
                            if (rowData[6] != "")
                                FechaFab = Convert.ToDateTime(rowData[6]);

                            DateTime? FechaVcto = null;
                            if (rowData[7] != "")
                                FechaVcto = Convert.ToDateTime(rowData[7]);

                            if (i_producto_padre.pr_codigo != cod_producto)
                            {
                                i_producto_padre = Bus_Producto.get_info_x_codigo(param.IdEmpresa, cod_producto);
                                if (i_producto_padre == null)
                                {
                                    MessageBox.Show("El código " + cod_producto + " no existe en la base de datos, por favor corrija");
                                    return false;
                                }
                            }
                            in_Producto_Info i_producto_lote = new in_Producto_Info
                            {
                                IdEmpresa = i_producto_padre.IdEmpresa,
                                IdProducto = 0,
                                pr_codigo = i_producto_padre.pr_codigo,
                                pr_codigo2 = i_producto_padre.pr_codigo2,
                                pr_descripcion = i_producto_padre.pr_descripcion,
                                pr_descripcion_2 = i_producto_padre.pr_descripcion_2,
                                IdProductoTipo = IdProducto_tipo_lote,
                                IdMarca = i_producto_padre.IdMarca,
                                IdPresentacion = i_producto_padre.IdPresentacion,
                                IdCategoria = i_producto_padre.IdCategoria,
                                IdLinea = i_producto_padre.IdLinea,
                                IdGrupo = i_producto_padre.IdGrupo,
                                IdSubGrupo = i_producto_padre.IdSubGrupo,
                                IdUnidadMedida = i_producto_padre.IdUnidadMedida,
                                IdUnidadMedida_Consumo = i_producto_padre.IdUnidadMedida_Consumo,
                                pr_codigo_barra = i_producto_padre.pr_codigo_barra,
                                pr_observacion = i_producto_padre.pr_observacion,
                                Estado = "A",
                                IdCod_Impuesto_Iva = i_producto_padre.IdCod_Impuesto_Iva,
                                Aparece_modu_Ventas = i_producto_padre.Aparece_modu_Ventas,
                                Aparece_modu_Activo_F = i_producto_padre.Aparece_modu_Activo_F,
                                Aparece_modu_Compras = i_producto_padre.Aparece_modu_Compras,
                                Aparece_modu_Inventario = i_producto_padre.Aparece_modu_Inventario,
                                IdProducto_padre = i_producto_padre.IdProducto,
                                lote_num_lote = codigo_lote,
                                lote_fecha_fab = FechaFab,
                                lote_fecha_vcto = FechaVcto,
                                precio_1 = i_producto_padre.precio_1,
                                precio_2 = i_producto_padre.precio_2,
                                precio_3 = i_producto_padre.precio_3,
                                precio_4 = i_producto_padre.precio_4,
                                precio_5 = i_producto_padre.precio_5,
                                porcentaje_2 = i_producto_padre.porcentaje_2,
                                porcentaje_3 = i_producto_padre.porcentaje_3,
                                porcentaje_4 = i_producto_padre.porcentaje_4,
                                porcentaje_5 = i_producto_padre.porcentaje_5,
                                signo_2 = i_producto_padre.signo_2,
                                signo_3 = i_producto_padre.signo_3,
                                signo_4 = i_producto_padre.signo_4,
                                signo_5 = i_producto_padre.signo_5,
                                IdUsuario = "NUEVO",
                                se_distribuye = false
                            };

                            if (!Bus_Producto.GrabarDB(i_producto_lote, ref IdProducto, ref mensajeErrorPantalla))
                            {
                                MessageBox.Show(mensajeErrorPantalla);
                                return false;
                            }
                            else
                                Info_Producto.IdProducto = i_producto_lote.IdProducto;
                        }
                        else
                            Info_Producto.IdProducto = ID_prod_lote;
                    }                    
        ...
        private void cargar_alerta(string CodAlerta)
        {
            try
            {
                ...
                info_alerta = bus_alerta.get_info(param.IdEmpresa, param.IdUsuario, CodAlerta, Cl_Enumeradores.eEventos_alerta.TRANSACCIONES.ToString());
                if (info_alerta == null)
                    return;
                ...
                {
                    MessageBox.Show("Ha ocurrido un error al cargar el ensamblado " + nombre_dll);
                    return;
                }
