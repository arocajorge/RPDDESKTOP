...
{
    public partial class frmFa_Factura_Mant_FJ : Form
    {
        ...
        Business.Facturacion.fa_factura_Bus Bus_Factura = new Business.Facturacion.fa_factura_Bus();
        Business.Facturacion.fa_factura_det_Bus Bus_factura_det = new Business.Facturacion.fa_factura_det_Bus();
        ...
        public frmFa_Factura_Mant_FJ()
        {
            InitializeComponent();
            event_frmFA_Factura_FormClosing += frmFa_Factura_Mant_FJ_event_frmFA_Factura_FormClosing;
        }

        void frmFa_Factura_Mant_FJ_event_frmFA_Factura_FormClosing(object sender, FormClosingEventArgs e)
        {

        }

        private void frmFa_Factura_Mant_FJ_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        private void Set_Info_in_controls()
        {
            ...
            {                
                ...
                /*
                //Campos de factura personalizada para fj
                InfoFactura.Factura_Fj = bus_factura_fj.Get_info_factura_fj(InfoFactura.IdEmpresa, InfoFactura.IdSucursal, InfoFactura.IdBodega, InfoFactura.IdCbteVta);
                txt_num_oc.Text = InfoFactura.Factura_Fj.num_oc;
                txt_atencion_a.Text = InfoFactura.Factura_Fj.Atencion_a;
                txt_referencia.Text = InfoFactura.Factura_Fj.descripcion_fact;
                de_fecha_cobro_1.EditValue = InfoFactura.Factura_Fj.fecha_cobro_1;
                de_fecha_cobro_2.EditValue = InfoFactura.Factura_Fj.fecha_cobro_2;
                */
        ...
        public  fa_factura_Info Get_Info_Factura()
        {
            try
            {
                ...
                //InfoFactura.Factura_Fj = Get_Info_Factura_fj();
        ...
        private fa_factura_fj_Info Get_Info_Factura_fj()
        {
            try
            {
                ...
                info_fj.IdCbteVta = txtIdFactura.Text == "" ? 0 : Convert.ToDecimal(txtIdFactura.Text);
                info_fj.num_oc = txt_num_oc.Text;
                info_fj.Atencion_a = txt_atencion_a.Text;
                info_fj.descripcion_fact = txt_referencia.Text;
                info_fj.fecha_cobro_1 = de_fecha_cobro_1.EditValue == null ? InfoFactura.vt_fech_venc : de_fecha_cobro_1.DateTime;
                info_fj.fecha_cobro_2 = de_fecha_cobro_2.EditValue == null ? InfoFactura.vt_fech_venc : de_fecha_cobro_2.DateTime;
                ...
                else
                {
                    ...
                    if (MessageBox.Show("La FACT # " + InfoFactura.vt_serie1 + "-" + InfoFactura.vt_serie2 + "-" + InfoFactura.vt_NumFactura + " se procedera a Guardar." + "\n" + "¿Desea Continuar?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        if (bolResult)
                        {
                            ...
                            {
                                /*
                                InfoFactura.Factura_Fj.IdCbteVta = idCbte_vta;
                                fa_factura_fj_Bus bus_fact_fj = new fa_factura_fj_Bus();
                                bus_fact_fj.GuardarDB(InfoFactura.Factura_Fj);
                                 * */
                            }
                            ...
                            {
                                ucGe_Menu_event_btn_Generar_XML_Click_1(null, null);
                            }
                            limpiar();
                        }
                        else
                ...
                else
                {
                    ...
                    if (MessageBox.Show("La FACT # " + InfoFactura.vt_serie1 + "-" + InfoFactura.vt_serie2 + "-" + InfoFactura.vt_NumFactura + " se procedera a Guardar." + "\n" + "¿Desea Continuar?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        if (bolResult)
                        {
                            ...
                            if (InfoFactura.IdCbteVta != 0)
                            {
                                /*
                                InfoFactura.Factura_Fj.IdCbteVta = InfoFactura.IdCbteVta;
                                fa_factura_fj_Bus bus_fact_fj = new fa_factura_fj_Bus();
                                bus_fact_fj.ModificarDB(InfoFactura.Factura_Fj);
                                 * */
                            }
        ...
        Boolean Modificar_proceso_cerrado()
        {
            try
            {
                Boolean bolResult = false;
                ...
                else
                {
                    ...
                    if (MessageBox.Show("La FACT # " + InfoFactura.vt_serie1 + "-" + InfoFactura.vt_serie2 + "-" + InfoFactura.vt_NumFactura + " se procedera a Guardar." + "\n" + "¿Desea Continuar?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        bolResult = Bus_Factura.ModificarDB_proceso_cerrado(InfoFactura, ref msg);
                        if (bolResult)
                        {
        ...
        public void ucGe_Menu_event_btnImprimirSoporte_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                rptSoporte.P_IdSucursal.Value = InfoFactura.IdSucursal;
                rptSoporte.P_IdSucursal.Visible = false;

                rptSoporte.P_IdBodega.Value = InfoFactura.IdBodega;
                rptSoporte.P_IdBodega.Visible = false;

                rptSoporte.P_IdCbteVta.Value = InfoFactura.IdCbteVta;
        ...
        void ImprimirDiario()
        {
            try
            {
                if (MessageBox.Show("¿Desea imprimir el comprobante contable de la factura?",param.Nombre_sistema,MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        public void frmFA_Factura_Load(object sender, EventArgs e)
        {
            try
            {
                de_fecha_cobro_1.EditValue = DateTime.Now;
                de_fecha_cobro_2.EditValue = DateTime.Now;
        ...
        public void cargarNumDoc()
        {
            try
            {
                if (_Accion == Cl_Enumeradores.eTipo_action.grabar || _Accion == Cl_Enumeradores.eTipo_action.duplicar)
                {
                    UCNumDoc.IdTipoDocumento = Cl_Enumeradores.eTipoDocumento_Talonario.FACT;
        ...
        private void gridView_Factura_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                if (e.Column == colCodigo_Producto)
                {

                    in_Producto_Info info_pro = Lista_producto.FirstOrDefault(q => q.IdProducto == (e.Value == null ? 0 : Convert.ToDecimal(e.Value)));
                    if (info_pro != null)
                    {
                        txt_referencia.Text += info_pro.pr_descripcion.Trim() + "\n";
                        gridView_Factura.SetRowCellValue(e.RowHandle, colPrecio, info_pro.pr_precio_publico);
                    }
                }
                ...
                {
                    if (Info_Fact_Det.IdPunto_cargo != null)
                    {
                        info_punto_cargo = bus_punto_cargo.Get_info_punto_Cargo_con_subcentro(param.IdEmpresa, Convert.ToInt32(Info_Fact_Det.IdPunto_cargo));
                        if (info_punto_cargo != null)
                        {
                            Info_Fact_Det.IdCentroCosto = info_punto_cargo.IdCentroCosto_Scc;
                            Info_Fact_Det.IdCentroCosto_sub_centro_costo = info_punto_cargo.IdCentroCosto_sub_centro_costo_Scc;
                            Info_Fact_Det.IdPunto_cargo_grupo = info_punto_cargo.IdPunto_cargo_grupo;
                        }
                        else
                        {
                            Info_Fact_Det.IdCentroCosto = null;
                            Info_Fact_Det.IdCentroCosto_sub_centro_costo = null;
                            Info_Fact_Det.IdPunto_cargo_grupo = null;
                        }
                    }
                    else
                    {
        ...
        private void dateFecha_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                int Dias = Convert.ToInt32(spinEditDiasPlazo.EditValue);
                dateFechaVencimiento.Value = dateFecha.Value.AddDays(Dias);
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
        ...
        private void dateFecha_ValueChanged_1(object sender, EventArgs e)
        {
            try
            {
                int dias = spinEditDiasPlazo.EditValue == null ? 0 : Convert.ToInt32(spinEditDiasPlazo.EditValue);
                dateFechaVencimiento.Value = Convert.ToDateTime(dateFecha.Value).AddDays(dias);
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
