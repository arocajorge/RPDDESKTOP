...
{
    public partial class frmFa_Nota_Cred_Mant : Form
    {
        ...
        Business.Facturacion_Grafinpren.fa_factura_graf_Bus bus_factura_graf = new Business.Facturacion_Grafinpren.fa_factura_graf_Bus();
        Info.Facturacion_Grafinpren.fa_factura_graf_Info info_factura_graf = new Info.Facturacion_Grafinpren.fa_factura_graf_Info();
        ...
        Core.Erp.Business.Facturacion_Grafinpren.fa_notaCreDeb_graf_Bus Bus = new Business.Facturacion_Grafinpren.fa_notaCreDeb_graf_Bus();
        ...
        void ucGe_Menu_event_btn_Generar_XML_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Bus.GenerarXml_notaCreDeb(param.IdEmpresa, Convert.ToInt32(ucSucursalBode.cmb_sucursal.EditValue), Convert.ToInt32(ucSucursalBode.cmb_bodega.EditValue), Convert.ToDecimal(txtNumNot.Text), CreDeb, param_info.pa_ruta_descarga_xml_fac_elct, ref  mensaje))
                {
        ...
        void ucGe_Menu_event_btnAnular_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(param.Get_Mensaje_sys(enum_Mensajes_sys.Está_seguro_que_desea_anular_la) +" Nota Cred "+ Info_NotaCre.IdTipoDoc + " ?", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (Bus.AnularDB(Info_NotaCre, ref mensajeError))
                    {
        ...
        Boolean grabarDatos()
        {
            try
            {
                ...
                if (bolResult)
                {
                    ...
                    GetInfoGraf();
                    //CobrosInfo = new cxc_cobro_Info();
                    //CobrosInfo = get_Cobro();
        ...
        public void cargarCombos()
        {
            try
            {
                ...
                cmb_subCetroCosoto.DataSource = list_subcentro_combo;
        ...
        public void GetInfo()
        {
            try
            {
                ...
                Info_NotaCre.no_fecha = Convert.ToDateTime(dtFechaNota.Value.ToShortDateString());
        ...
        private void GetInfoGraf()
        {
            try
            {
                Info_NotaCre.NotaCreDeb_Graf_Info.IdEmpresa = Info_NotaCre.IdEmpresa;
                Info_NotaCre.NotaCreDeb_Graf_Info.IdSucursal = Info_NotaCre.IdSucursal;
                Info_NotaCre.NotaCreDeb_Graf_Info.IdBodega = Info_NotaCre.IdBodega;
                Info_NotaCre.NotaCreDeb_Graf_Info.IdNota = Info_NotaCre.IdNota;
                Info_NotaCre.NotaCreDeb_Graf_Info.num_op = (txt_num_op.Text == "") ? " " : txt_num_op.Text;
                Info_NotaCre.NotaCreDeb_Graf_Info.fecha_op = dt_fecha_num_op.Value;
                Info_NotaCre.NotaCreDeb_Graf_Info.num_cotizacion = (txt_num_cotizacion.Text == "") ? " " : txt_num_cotizacion.Text;
                Info_NotaCre.NotaCreDeb_Graf_Info.fecha_cotizacion = dt_fecha_cotizacion.Value;
                Info_NotaCre.NotaCreDeb_Graf_Info.IdEquipo = cmbEquipo.Get_Equipo().IdEquipo;
                Info_NotaCre.NotaCreDeb_Graf_Info.porc_comision = (txt_porc_comision.Text == "") ? 0 : Convert.ToDouble(txt_porc_comision.Text);
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
        ...
        private void Set()
        {
            try
            {
                ...
                dtFechaNota.Value = (DateTime)Info_NotaCre.no_fecha;
                ...
                if (Info_NotaCre.NaturalezaNota == "SRI")
                {
                    ...
                    InfoTalonario_Grabado = BusTalonario.Get_Info_Documento_Tipo_Talonario(Info_NotaCre.IdEmpresa, Info_NotaCre.CodDocumentoTipo
                        , Info_NotaCre.Serie1, Info_NotaCre.Serie2, Info_NotaCre.NumNota_Impresa);
        ...
        private void SetDetalle()
        {
            try
            {
                ...
                Info_NotaCre.ListaDetalles = Bus.Get_List_notaCreDeb_det(Info_NotaCre);
        ...
        private void SetInfoGraf()
        {
            try
            {
                if (Info_NotaCre.NotaCreDeb_Graf_Info.IdNota != 0)
                {
                    txt_num_op.Text = Info_NotaCre.NotaCreDeb_Graf_Info.num_op==null ? "" : Info_NotaCre.NotaCreDeb_Graf_Info.num_op.ToString();
                    txt_num_cotizacion.Text = Info_NotaCre.NotaCreDeb_Graf_Info.num_cotizacion == null ? "" : Info_NotaCre.NotaCreDeb_Graf_Info.num_cotizacion.ToString();
                    cmbEquipo.Set_InfoEquipo((Info_NotaCre.NotaCreDeb_Graf_Info.IdEquipo == null) ? 0 : Convert.ToInt32(Info_NotaCre.NotaCreDeb_Graf_Info.IdEquipo));
                    dt_fecha_num_op.Value = Info_NotaCre.NotaCreDeb_Graf_Info.fecha_op == null ? DateTime.Now : Convert.ToDateTime(Info_NotaCre.NotaCreDeb_Graf_Info.fecha_op);
                    dt_fecha_cotizacion.Value = Info_NotaCre.NotaCreDeb_Graf_Info.fecha_cotizacion == null ? DateTime.Now : Convert.ToDateTime(Info_NotaCre.NotaCreDeb_Graf_Info.fecha_cotizacion);
                    txt_porc_comision.Text = Info_NotaCre.NotaCreDeb_Graf_Info.porc_comision == null ? "" : Info_NotaCre.NotaCreDeb_Graf_Info.porc_comision.ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        Boolean Guardar()
        {
            try
            {
                ...
                //if (Validar())
                {
                    ...
                    {
                        if (Bus.VerificarCodigo(txtCodigoNot.Text) == false)
                        {
                            Info_NotaCre.Info_sisDocTipoTalo = talonarioInfo;
                            if (Bus.GrabarDB(Info_NotaCre, ref Idnota,ref mensajeDocumentoDupli, ref mensajeError))
                            {
        ...
        Boolean Actualizar()
        {
            try
            {
                ...
                //if (Validar())
                {
                    ...
                    if (Bus.ModificarDB(Info_NotaCre, ref mensajeError, ref mensajeDocumentoDupli))
                    {
                        bolResult = true;
        ...
        void LimpiarDatos()
        {
            try
            {
                ...
                txtNumNot.Text = "";           
                txtCodigoNot.Text = "";
                dtFechaNota.Value = DateTime.Now;
        ...
        private Boolean Validar()
        {
            ...
            try
            {
                ...
                //if (txt_num_op.Text == "" || txt_num_op.Text == null)
                //{
                //    MessageBox.Show(param.Get_Mensaje_sys(enum_Mensajes_sys.Por_Favor_ingrese_la) + " OP ", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                //    txt_num_op.Focus();
                //    return false;
                //}


                //if (Info_NotaCre.ListaDetalles.Count() == 0)
                ...
                foreach (var item in BList_Documentos_relacionados)
                {
                    ...
                    if (Math.Round( item.Valor_Aplicado,2,MidpointRounding.AwayFromZero) > Math.Round( Convert.ToDouble(item.vt_total==null ? 0 :  item.vt_total),2,MidpointRounding.AwayFromZero) )
                    {
                ...
                if (cmb_vendedor.get_VendedorInfo() == null || cmb_vendedor.get_VendedorInfo().IdVendedor == 0)
                {
                    MessageBox.Show("Seleccione un vendedor.", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }

                double Total_CR = 0;
                double Total_NC = 0;
                Total_NC = Math.Round(BindiList_det_NC.Sum(q => q.sc_total), 2);
                Total_CR = Math.Round(BList_Documentos_relacionados.Sum(q => q.Valor_Aplicado) == null ? 0 : (double)BList_Documentos_relacionados.Sum(q => q.Valor_Aplicado), 2);
                if (Total_CR != Total_NC)
                {
                    MessageBox.Show("Existe una diferencia en los valores ingresados \nEl valor total de los documentos relacionados es: " + Total_CR + "\nEl valor total de la Nota de crédito es: " + Total_NC + "\n Por favor corrija los valores.", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }

                if (!param.Validar_periodo_cerrado_x_modulo(param.IdEmpresa, Cl_Enumeradores.eModulos.FAC, Convert.ToDateTime(dtFechaNota.Value)))
                    return false;
                            ...
                            if (Info_Doc_rel != null && Info_Doc_rel.IdCbteVta_fac_nd_doc_mod != 0)
                            {
                                switch (Info_Doc_rel.vt_tipoDoc)
                                {
                                    case "FACT":
                                        ...
                                        info_fact.Factura_Graf = bus_factura_graf.Get_Info_graf(Info_Doc_rel.IdEmpresa_fac_nd_doc_mod, Info_Doc_rel.IdSucursal_fac_nd_doc_mod, Info_Doc_rel.IdBodega_fac_nd_doc_mod, Info_Doc_rel.IdCbteVta_fac_nd_doc_mod);
                                        if (info_fact.Factura_Graf!=null)
                                        {
                                            if (info_fact.Factura_Graf.IdCbteVta >0)
                                            {
                                                txt_num_cotizacion.Text = info_fact.Factura_Graf.num_cotizacion;
                                                txt_porc_comision.Text = info_fact.Factura_Graf.porc_comision.ToString();
                                                txt_num_op.Text = info_fact.Factura_Graf.num_op;
                                                dt_fecha_cotizacion.Value = info_fact.Factura_Graf.fecha_cotizacion;
                                                dt_fecha_num_op.Value = info_fact.Factura_Graf.fecha_op;
                                                cmbEquipo.Set_InfoEquipo(info_fact.Factura_Graf.IdEquipo);
                                            }
                                        }
                                         * */
                                        ...
                                        {                                            
                                            ...
                                            foreach (var item in info_fact.DetFactura_List)
                                            {
                                                ...
                                                info.vt_por_iva = item.vt_por_iva == null ? 0 : Convert.ToDouble(item.vt_por_iva);
        ...
        private void cmb_subCetroCosoto_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Llamar_a_pantalla_subcentro()
        {
            try
            {
                ...
                fa_notaCreDeb_det_Info Row = (fa_notaCreDeb_det_Info)gridViewNotaCreDeb.GetRow(rowHandle);
                if (Row != null)
                {
                    if (Row.IdCentroCosto != null)
                    {
                        ...
                        if (Lista_subcentro_consulta != null && Lista_subcentro_consulta.Count != 0)
                        {
                            ...
                            gridViewNotaCreDeb.SetRowCellValue(rowHandle, Col_IdCentroCosto_sub_centro_costo, info_subcentro.IdCentroCosto_sub_centro_costo);
                        }
                    }
                }
            }
