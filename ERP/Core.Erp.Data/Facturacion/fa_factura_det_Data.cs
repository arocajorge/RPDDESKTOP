  ...
  public  class fa_factura_det_Data
    {
              ...
              using (EntitiesFacturacion Contex = new EntitiesFacturacion())
              {
                  ...
                  foreach (var item in Lista)
                  {
                      if (item.IdProducto != 0)
                      {
                          ...
                          address.IdEmpresa_pf = item.IdEmpresa_pf;
                          address.IdSucursal_pf = item.IdSucursal_pf;
                          address.IdProforma = item.IdProforma;
                          address.Secuencia_pf = item.Secuencia_pf;
      ...
      public List<fa_factura_det_info> Get_List_factura_det(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdCbteVta, ref string msg)
      {
          try
          {
              ...
              foreach (var item in consulta)
              {
                  ...
                  info.IdEmpresa_pf = item.IdEmpresa_pf;
                  info.IdSucursal_pf = item.IdSucursal_pf;
                  info.IdProforma = item.IdProforma;
                  info.Secuencia_pf = item.Secuencia_pf;
      ...
      public List<fa_factura_det_info> Get_List_factura_para_guia(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdCbteVta, ref string msg)
      {
          try
          {
              List<fa_factura_det_info> Lista = new List<fa_factura_det_info>();
      ...
      public bool eliminarDB(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdCbteVta)
      {
          try
          {
              using (EntitiesFacturacion Context = new EntitiesFacturacion())
              {
                  Context.Database.ExecuteSqlCommand("delete fa_factura_det where IdEmpresa = "+IdEmpresa+" and IdSucursal = "+IdSucursal+" and IdBodega = "+IdBodega+" and IdCbteVta = "+IdCbteVta);
              }
