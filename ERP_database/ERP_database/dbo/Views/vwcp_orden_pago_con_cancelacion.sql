...
FROM            (SELECT        A.IdEmpresa, A.IdTipo_op, SUBSTRING(D .Descripcion, 1, 3) + '#' + C.co_serie + ' -' + C.co_factura AS Referencia, SUBSTRING(D .Descripcion, 1, 3) + '#' + CAST(CAST(C.co_factura AS NUMERIC) AS VARCHAR) 
                                                    ...
                                                    B.IdEstadoAprobacion, B.IdFormaPago, B.Fecha_Pago, vwct_cbtecble_con_ctacble_acreedora.IdCtaCble_Acreedora AS IdCtaCble, C.IdCentroCosto, NULL AS IdSubCentro_Costo, C.IdCbteCble_Ogiro AS Cbte_cxp, 
                          ...
                          SELECT        C.IdEmpresa, 'FACT_PROVEE' AS IdTipo_op, SUBSTRING(D .Descripcion, 1, 3) + '#' + C.co_serie + ' -' + C.co_factura AS Referencia, 'FA#:' + CAST(CAST(C.co_factura AS NUMERIC) AS VARCHAR(20)) 
                                                   ...
                                                   'APRO' AS IdEstadoAprobacion, 'EFEC' AS IdFormaPago, NULL AS Fecha_Pago, vwct_cbtecble_con_ctacble_acreedora.IdCtaCble_Acreedora AS IdCtaCble, C.IdCentroCosto, NULL AS IdSubCentro_Costo, 
