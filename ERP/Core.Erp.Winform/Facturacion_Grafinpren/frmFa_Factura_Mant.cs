    ...
    public partial class frmFa_Factura_Mant : Form
    {
        ...
        private void gridView_Factura_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                if (colIdCod_Impuesto_Iva == e.Column)
                {
                    tb_sis_impuesto_Info info_impuesto = new tb_sis_impuesto_Info();
                    info_impuesto = listTipoImpu_x_Iva.FirstOrDefault(q => q.IdCod_Impuesto == (e.Value == null ? "" : e.Value.ToString()));
                    if (info_impuesto != null)
                    {
                        Info_Fact_Det.IdCod_Impuesto_Iva = info_impuesto.IdCod_Impuesto;
                        Info_Fact_Det.vt_por_iva = info_impuesto.porcentaje;
                    }
                    else
                    {
                        Info_Fact_Det.IdCod_Impuesto_Iva = param.iva.IdCod_Impuesto;
