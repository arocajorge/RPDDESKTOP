        ...
        */
        ...
        public List<ct_Plancta_Info> Get_List_Plancta_x_ctas_Movimiento(int IdEmpresa, ref string MensajeError,Boolean Mostrar_Todo_El_Plan_cta=false)
        {
            ...
            try
            {
                ...
                {
                    lM = (from C in OEselectPlancta.vwct_plancta
                          where C.IdEmpresa == IdEmpresa
                          select new ct_Plancta_Info
                          {
                              IdCtaCble = C.IdCtaCble,
                              pc_Cuenta = C.pc_Cuenta,
                              
                              IdEmpresa = C.IdEmpresa,
                              IdCtaCblePadre = C.IdCtaCblePadre,
                              pc_Naturaleza = C.pc_Naturaleza,
                              IdNivelCta = C.IdNivelCta,
                              IdGrupoCble = C.IdGrupoCble,
                              pc_Estado = C.pc_Estado,
                              pc_EsMovimiento = C.pc_EsMovimiento,
                              pc_clave_corta = C.pc_clave_corta,
                              CuentaPadre = C.CuentaPadre,
                              SEstado = (C.pc_Estado == "A") ? "ACTIVO" : "*ANULADO*",
                          }).ToList();
                }
                else
                {
                ...
                lM.ForEach(C => C.pc_Cuenta2 = ClaveCorta + "[" + C.IdCtaCble + "] - " + C.pc_Cuenta);
                return lM;
            }
            catch (Exception ex)
            {
