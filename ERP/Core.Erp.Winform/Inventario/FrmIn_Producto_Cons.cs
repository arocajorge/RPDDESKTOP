       ...
       public in_Producto_Info Get_Info_Producto()
       {
        ...
        private void load_grid()
        {
            try
            {
                ...
                list_Info_Producto = Bus_Producto.get_list_producto_consulta(param.IdEmpresa);
        ...
        private void cmb_codigo_barra_Click(object sender, EventArgs e)
        {
            try
            {
                in_Producto_Info row = (in_Producto_Info)gridViewProducto.GetFocusedRow();
                if (row == null) return;
                XINV_RptCodigo_Barra rpt = new XINV_RptCodigo_Barra();
                List<in_Producto_Info> lst_producto = new List<in_Producto_Info>();
                lst_producto.Add(row);
                rpt.lst_producto = lst_producto;
