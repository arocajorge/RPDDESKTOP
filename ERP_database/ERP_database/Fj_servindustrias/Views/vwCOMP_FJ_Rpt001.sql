...
SELECT        OC.IdEmpresa, OC.IdSucursal, OC.IdOrdenCompra, OC.IdProveedor, OC.oc_NumDocumento, OC.Tipo, OC.IdTerminoPago, OC.oc_plazo AS Plazo, OC.oc_fecha AS Fecha, OC.oc_flete AS Flete, 
                         OC.oc_observacion AS Observacion, OC.Estado, OC.IdSolicitante, OC.IdComprador, OC.IdDepartamento, depar.de_descripcion AS departamento, OC_det.Secuencia, OC_det.IdProducto, 
                         ...
                         OC_det.do_total AS total, OC_det.do_peso AS peso, Prod.pr_codigo AS cod_producto, Prod.pr_descripcion AS nom_producto, sucu.Su_Descripcion AS sucursal, empr.em_nombre AS empresa, 
...
FROM            dbo.com_solicitante RIGHT OUTER JOIN
                         ...
                         dbo.com_solicitante.IdEmpresa = OC.IdEmpresa AND dbo.com_solicitante.IdSolicitante = OC.IdSolicitante LEFT OUTER JOIN
