...
namespace Core.Erp.Winform.Roles
{
    public partial class frmRo_Empleado_Mant : Form
    {
        ...
        BindingList<ro_empleado_x_rubro_acumulado_Info> oBindingListro_rubro_tipo_Info = new BindingList<ro_empleado_x_rubro_acumulado_Info>();
        BindingList<ro_rubro_tipo_Info> lista_rubro = new BindingList<ro_rubro_tipo_Info>();

        ro_rubro_tipo_Bus bus_rubro = new ro_rubro_tipo_Bus();
        ro_empleado_x_rubro_acumulado_Bus oRo_ConfRubrosAcumulados_Bus = new ro_empleado_x_rubro_acumulado_Bus();
...
#region DETALLES DEL EMPLEADO
                ...
                if(infoEmpleado!=null){
                   ...
                   //List<ro_empleado_x_titulos_Info> oListRo_empleado_x_titulos_Info = new List<ro_empleado_x_titulos_Info>();
                    ...
                    if (Accion == Cl_Enumeradores.eTipo_action.grabar)
                        lista_rubro = new BindingList<ro_rubro_tipo_Info>();

                    foreach (ro_rubro_tipo_Info item in lista_rubro)
                    {
        ...
        public void set_Empleado(ro_Empleado_Info info)
        {
            try
            {
                ...
                {
                    foreach (var item2 in oBindingListro_rubro_tipo_Info)
                        {
                  ...
                  lblAnulado.Visible = false;
                    ...
                    switch (Accion)
                    {
                        case Cl_Enumeradores.eTipo_action.grabar:
                            lista_rubro = new BindingList<ro_rubro_tipo_Info>(bus_rubro.Get_List_rubros_acumulado(param.IdEmpresa));
                            gridRubroAcum.DataSource = lista_rubro;
                        ...
                        case Cl_Enumeradores.eTipo_action.actualizar:
                           ...
                           // busco los rubros acumulados por empleados
                            oBindingListro_rubro_tipo_Info = new BindingList<ro_empleado_x_rubro_acumulado_Info>(oRo_ConfRubrosAcumulados_Bus.Get_List_empleado_x_rubro_acumulado(param.IdEmpresa, infoEmpleado.IdEmpleado));

                            foreach (var item in lista_rubro)
                            {
                                foreach (var itemR in oBindingListro_rubro_tipo_Info)
                                {
        ...
        public Boolean pu_Validar()
        {
            try
            {
                ...
                if (Accion == Cl_Enumeradores.eTipo_action.grabar)
                {
                    if (ListHsueldo.Count() == 0)
                    {
                        MessageBox.Show("Ingrese sueldo actual Para el empleado", "ATENCION", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return false;
                    }
                    if (ListHsueldo.Sum(v => v.SueldoActual) == 0)
                    {
                        MessageBox.Show("Ingrese sueldo actual Para el empleado", "ATENCION", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return false;
                    }
        ...
        private void Limpiar()
        {
            try
            {
                ...
                //oBindingListro_rubro_tipo_Info = new BindingList<ro_empleado_x_ro_rubro_Info>(bus_rubro.Get_List_rubros_acumulado(param.IdEmpresa).Where(v => v.rub_concep == true).ToList());
                oBindingListRo_Horario_Info = new BindingList<ro_Horario_Info>(oRo_Horario_Bus.Get_List_Horario(param.IdEmpresa));
                oBindingListRo_Nomina_Tipo_Info = new BindingList<ro_Nomina_Tipo_Info>(oRo_Nomina_Tipo_Bus.Get_List_Nomina_Tipo(param.IdEmpresa));
                cmbNomina.DataSource = oListRo_Nomina_Tipo_Info;
                cmbNominaLiqui.DataSource = oListRo_Nomina_Tipoliqui_Info;

                //oBindingListro_rubro_tipo_Info = new BindingList<ro_rubro_tipo_Info>(bus_rubro.ConsultaGeneral(param.IdEmpresa).Where(v => v.rub_concep == true).ToList());
                gridRubroAcum.DataSource = oBindingListro_rubro_tipo_Info;
       ...
       public Boolean pu_Grabar(ref decimal idEmpleado)
        ...
        private Boolean pu_ValidarItemRubroFijo() { 
            try{
                ...
                if (ucRo_NominaTipo.getIdNominaTipo() != null && ucRo_NominaTipoLiqui.getNominaTipoLiqui()!=null &&  cmbRubro.EditValue!=null)
                {
                    valorDevolver = true;
                }
