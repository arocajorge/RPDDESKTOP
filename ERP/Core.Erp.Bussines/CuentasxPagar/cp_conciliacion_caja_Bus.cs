...
namespace Core.Erp.Business.CuentasxPagar
{
   public class cp_conciliacion_caja_Bus
   {
    ...
    item.Valor_a_aplicar);
               ...
               if (Info_Conciliacion.Detalle_x_ValeCaja.Count() > 0)
               {
                   ...
                   foreach (var item in Info_Conciliacion.Detalle_x_ValeCaja)
                   {
                       if (!item.Esta_en_base)
                       {
                           ...
                           movimiento_caja_info.IdPersona = item.IdPersona == null ? 0 : Convert.ToInt32(item.IdPersona);
                           ...
                           movimiento_caja_info.cm_observacion = "caj # " + IdConciliacion_Caja_GB.ToString() +" "+ item.cm_observacion;
                           ...
                           info_mov = bus_mov_caja.Get_Info_Movimiento_Tipo(item.IdTipoMovi == null ? 0 : Convert.ToInt32(item.IdTipoMovi), item.IdEmpresa, ref MensajeError);
       ...
       public string Eliminar_facturas_o_vales(int IdEmpresa, int IdTipoCbte, decimal IdCbteCble, string TipoCbte, bool Eliminar_desvincular, decimal IdConciliacion)
       {
           try
           {
               string MensajeError = "";
               decimal IdCbteAnu = 0;
               cp_conciliacion_Caja_det_x_ValeCaja_Bus bus_det_vale = new cp_conciliacion_Caja_det_x_ValeCaja_Bus();
               cp_conciliacion_Caja_det_Bus bus_det_fact = new cp_conciliacion_Caja_det_Bus();
               if (Eliminar_desvincular)
               {
                   if (IdCbteCble == 0)
                   {
                       return "eliminar_linea";
                   }

                   if (TipoCbte == "FACTURA")
                   {
                       cp_orden_giro_Bus bus_og = new cp_orden_giro_Bus();
                       cp_orden_giro_Info info_og = bus_og.Get_Info_orden_giro(IdEmpresa, IdTipoCbte, IdCbteCble);
                       if (info_og == null || info_og.IdEmpresa == 0)
                           return "eliminar_linea";

                       cp_orden_pago_det_Bus bus_op_det = new cp_orden_pago_det_Bus();
                       if (bus_op_det.validar_existe_op_x_og(IdEmpresa,IdTipoCbte,IdCbteCble))
                       {
                           return "Tiene vinculada una orden de pago, primero anule la OP asociada";
                       }

                       info_og.IdUsuarioUltAnu = param.IdUsuario;
                       info_og.MotivoAnu = "anulado desde conciliacion de caja";
                       info_og.Fecha_UltAnu = DateTime.Now;

                       cp_parametros_Bus bus_param_cp = new cp_parametros_Bus();
                       cp_parametros_Info info_param_cp = bus_param_cp.Get_Info_parametros(param.IdEmpresa);

                       info_og.IdTipoCbte_Anulacion = info_param_cp.pa_TipoCbte_OG_anulacion;
                       if (bus_og.AnularFacturaProveedor(info_og, new List<cp_orden_giro_x_com_ordencompra_local_Info>(), ref IdCbteAnu, ref MensajeError))
                       {
                           if (IdConciliacion == 0)
                           {
                               return "eliminar_linea";
                           ...
                           if (bus_det_fact.desvincularDB(IdEmpresa, IdTipoCbte, IdCbteCble, IdEmpresa, IdConciliacion))
                               return "eliminar_linea";
                       }
                       else
                           return MensajeError;
                   ...
                   else
                   {
                       ...
                       caj_Caja_Movimiento_Info info_mov = bus_mov.Get_Info_MovimientoCaja(IdEmpresa, IdCbteCble, IdTipoCbte, ref MensajeError);
                       if (info_mov == null || info_mov.IdEmpresa == 0)
                           return "eliminar_linea";

                       ct_Cbtecble_Bus CbteCble_B = new Contabilidad.ct_Cbtecble_Bus();
                       caj_parametro_Bus bus_param_Caj = new caj_parametro_Bus();
                       caj_parametro_Info info_param_caj = bus_param_Caj.Get_Info_Parametro(param.IdEmpresa);
                       int IdTipoCbteRev = info_param_caj.IdTipoCbteCble_MoviCaja_Egr_Anu;
                       decimal IdCbteCbleRev = 0;
                       if (CbteCble_B.ReversoCbteCble(param.IdEmpresa, info_mov.IdCbteCble, info_mov.IdTipocbte, IdTipoCbteRev, ref IdCbteCbleRev, ref MensajeError, param.IdUsuario))
                       {
                           info_mov.MotivoAnulacion = "Anulado desde conciliación de caja";
                           info_mov.IdUsuario_Anu = param.IdUsuario;
                           info_mov.FechaAnulacion = param.Fecha_Transac;
                           if (bus_mov.AnularDB(info_mov, ref MensajeError))
                           {
                               if (bus_det_vale.desvincularDB(IdEmpresa, IdConciliacion, IdEmpresa, IdTipoCbte, IdCbteCble))
                                   return "eliminar_linea";
                           }
                           else
                               return MensajeError;
...
;
       ...
       public bool Crear_ingreso_x_reposicion(cp_conciliacion_caja_Info info_conci, ref string error)
       {
           try
           {
               ...
               movimiento_caja_info.cm_observacion = "caj # " + info_conci.IdConciliacion_Caja.ToString() + " INGRESO_X_REPOSICION " + info_conci.Observacion;
