...
FROM            dbo.fa_proforma INNER JOIN
                         ...
                         dbo.tb_sucursal ON dbo.fa_proforma.IdEmpresa = dbo.tb_sucursal.IdEmpresa AND dbo.fa_proforma.IdSucursal = dbo.tb_sucursal.IdSucursal 
                         left join(
						 ...
						 group by d.IdEmpresa, d.IdSucursal, d.IdProforma
						 ) as pdet on dbo.fa_proforma.IdEmpresa = pdet.IdEmpresa AND dbo.fa_proforma.IdSucursal = pdet.IdSucursal AND 
                         dbo.fa_proforma.IdProforma = pdet.IdProforma
						 LEFT JOIN(
						 select fd.IdEmpresa_pf, fd.IdSucursal_pf, fd.IdProforma, sum(fd.vt_cantidad)vt_cantidad from fa_factura_det as fd
						 group by fd.IdEmpresa_pf, fd.IdSucursal_pf, fd.IdProforma
						 ) as dfet on dbo.fa_proforma.IdEmpresa = dfet.IdEmpresa_pf AND dbo.fa_proforma.IdSucursal = dfet.IdSucursal_pf AND 
