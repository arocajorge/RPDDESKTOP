    ...
    public class ct_Cbtecble_det_Data
    {
             ...
             try
             {
                 using (EntitiesDBConta contexto = new EntitiesDBConta())
                 {
                     ...
                     if (lst.Count()== 0)
                     {
                         ...
                         if (_CbteCbleInfo.IdPunto_cargo == 0)
                         {
                             address_tabla.IdPunto_cargo = null;
                         }else
                             address_tabla.IdPunto_cargo = _CbteCbleInfo.IdPunto_cargo;

                         if (_CbteCbleInfo.IdPunto_cargo_grupo == 0)
                         {
                             address_tabla.IdPunto_cargo_grupo = null;
                         }
                         else
        ...
        public List<ct_Cbtecble_det_Info> Get_list_CbteCble_x_cp_Conciliacion_caja(int IdEmpresa, decimal IdConciliacion_caja)
        {
            try
            {
                ...
                using (EntitiesDBConta Context = new EntitiesDBConta())
                {
                    Lista = (from q in Context.vwct_cbtecble_x_cp_Conciliacion_caja
                             where IdEmpresa == q.IdEmpresa &&
                             IdConciliacion_caja == q.IdConciliacion_Caja
                             select new ct_Cbtecble_det_Info
                             {
                                 IdEmpresa_conci = q.IdEmpresa,
                                 IdConciliacion_Caja = q.IdConciliacion_Caja,
                                 tipo = q.tc_TipoCbte,
                                 IdEmpresa = q.IdEmpresa_cbte,
                                 IdTipoCbte = q.IdTipoCbte,
                                 IdCbteCble = q.IdCbteCble,
                                 secuencia = q.secuencia,
                                 IdCtaCble = q.IdCtaCble,
                                 NomCtaCble = q.nom_Cuenta,
                                 IdCentroCosto = q.IdCentroCosto,
                                 NomCentroCosto = q.nom_Centro_costo,
                                 IdCentroCosto_sub_centro_costo = q.IdCentroCosto_sub_centro_costo,
                                 NomSubCentroCosto = q.nom_Centro_costo_sub_centro_costo,
                                 dc_Valor_D = q.Debe,
                                 dc_Valor_H = q.Haber,
