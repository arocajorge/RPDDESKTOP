  ...
  public  class in_Ing_Egr_Inven_Data
    {
      ...
      public bool GuardarDB_aprobar_contabilizar(in_Ing_Egr_Inven_Info info)
      {
          try
          {
              using (EntitiesInventario Context = new EntitiesInventario())
              {
                  ...
                  foreach (var item in info.listIng_Egr)
                  {
                      in_Ing_Egr_Inven_det Entity_det = new in_Ing_Egr_Inven_det
                      {
                          ...
                          IdMovi_inven_tipo = item.IdMovi_inven_tipo = info.IdMovi_inven_tipo,
                          IdNumMovi = item.IdNumMovi = info.IdNumMovi,
                          Secuencia = secuencia++,
                          IdBodega = item.IdBodega == 0 ? Convert.ToInt32(info.IdBodega) : Convert.ToInt32(item.IdBodega),
                          IdProducto = item.IdProducto,
                          dm_cantidad = info.signo == "+" ? Math.Abs(item.dm_cantidad) : Math.Abs(item.dm_cantidad) * -1,
                          ...
                          dm_cantidad_sinConversion = info.signo == "+" ? Math.Abs(item.dm_cantidad_sinConversion) : Math.Abs(item.dm_cantidad_sinConversion) * -1,
              ...
              aprobacion_sp(info.IdEmpresa, info.IdSucursal, info.IdBodega == null ? 0 : Convert.ToInt32(info.IdBodega), info.IdMovi_inven_tipo, info.IdNumMovi);
              return true;
          }
          catch (Exception)
          {
      ...
      public bool ModificarDB_aprobar_contabilizar(in_Ing_Egr_Inven_Info info)
      {
          try
          {
              ...
              odata_det.EliminarDB(info.IdEmpresa, info.IdSucursal,info.IdMovi_inven_tipo, info.IdNumMovi, ref mensaje);
              using (EntitiesInventario Context = new EntitiesInventario())
              {
      ...
      public bool aprobacion_sp(int IdEmpresa, int IdSucursal, int IdBodega, int IdMovi_inven_tipo, decimal IdNumMovi)
      {
          try
          {
              using (EntitiesInventario Context = new EntitiesInventario())
              {
                  decimal ID = Context.spINV_aprobacion_ing_egr(IdEmpresa, IdSucursal, IdBodega, IdMovi_inven_tipo, IdNumMovi);
              }
          ...
          try
          {
              ...
              Query = from q in oEnti.vwin_Ing_Egr_Inven
                  ...
                  foreach (var item in Query)
                  {
                      ...
                      Obj.pe_apellido = item.pe_apellido + " " + item.pe_nombre;
