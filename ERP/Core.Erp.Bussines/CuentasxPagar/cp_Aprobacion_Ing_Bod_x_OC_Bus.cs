  ...
  public  class cp_Aprobacion_Ing_Bod_x_OC_Bus
    {
      ...
      public Boolean GuardarDB(cp_Aprobacion_Ing_Bod_x_OC_Info Info, ref decimal id, ref string msg)
      {
          try
          {
              ...
              {
                  //Si los movimientos en el detalle no tienen numero, se crea el movimiento
                  if (Info.info_ing_x_compra.IdNumMovi == 0 && Info.info_ing_x_compra.listIng_Egr.Count >0)
                  {
                      decimal IdNumMovi = 0;
                      if (bus_ing_egr.GuardarDB(Info.info_ing_x_compra, ref IdNumMovi, ref mensaje))
                      {
                          Info.info_ing_x_compra.IdNumMovi = IdNumMovi;
                          int sec = 1;
                          foreach (var item in Info.listDetalle)
                          {
                              item.IdEmpresa_Ing_Egr_Inv = Info.info_ing_x_compra.IdEmpresa;
                              item.IdSucursal_Ing_Egr_Inv = Info.info_ing_x_compra.IdSucursal;
                              item.IdMovi_inven_tipo_Ing_Egr_Inv = Info.info_ing_x_compra.IdMovi_inven_tipo;
                              item.IdNumMovi_Ing_Egr_Inv = IdNumMovi;
                              item.Secuencia_Ing_Egr_Inv = sec;
                              sec++;
                          }
                      }
                  }
