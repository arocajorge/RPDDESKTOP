...
namespace Core.Erp.Winform.ActivoFijo
{
    public partial class frmAF_ActivoFijo_ImportWizard : Form
    {
        ...
        Af_Activo_fijo_tipo_Bus tipo_bus = new Af_Activo_fijo_tipo_Bus();
        ...
        public bool PU_CARGA_EXCEL()
        {
            try
            {
                if (CargarArchivoExcelADataTable())
                {
                    ...
                    lista_categoria = categoria_bus.ProcesarDataTableAf_Activo_fijo_Categoria_Info(DataTable_DATA_Categoria, param.IdEmpresa, ref MensajeError);

                    //Tipo af
                    DataTable_DATA_Tipo = Cargar_ArchivoExcel_x_Tipo_A_DataTable();
                    lista_tipo = tipo_bus.ProcesarDataTableAf_Activo_fijo_tipo_Info(DataTable_DATA_Tipo, param.IdEmpresa, ref MensajeError);
        ...
        {
            DataTable DataTable_DATA_Categoria = new DataTable();
            try
            {
        ...
        DataTable Cargar_ArchivoExcel_x_Tipo_A_DataTable()
        {
             DataTable_DATA_Tipo = new DataTable();
            try
            {
                DataTable_DATA_Tipo.Clear();
                ...
                OleDbConnection mconn = new OleDbConnection(cb.ConnectionString);
                //El nombre de la hoja del archivo de marcaciones tiene que llamarse Marcaciones
                OleDbDataAdapter ad = new OleDbDataAdapter("Select * from [Tipo$]", mconn);
                //abre una conexion de tipo oledb
                mconn.Open();
                //Lo agrega a mi datatable
                ad.Fill(DataTable_DATA_Tipo);

                mconn.Close();
                return DataTable_DATA_Tipo;
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
