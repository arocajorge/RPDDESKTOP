...
namespace Core.Erp.Winform.CuentasxCobrar
{
    public partial class frmCxc_CobrosGenerales_Mant : Form
    {
        ...
        private void cmb_cobros_multiples_Click(object sender, EventArgs e)
        {
            try
            {
                vwcxc_cartera_x_cobrar_Info row_det = (vwcxc_cartera_x_cobrar_Info)gridViewCobros.GetFocusedRow();
                if (row_det == null)
                    return;

                if (row_det.TipoDoc == "NTDB")
                {
                    MessageBox.Show("Los cobros múltiples son solo para facturas",param.Nombre_sistema,MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
                    return;
                }
                fa_factura_Bus busFac = new fa_factura_Bus();
                frmCxc_cobros_x_factura frm_m = new frmCxc_cobros_x_factura();
                fa_factura_Info row_factura = busFac.Get_Info_factura(row_det.IdEmpresa, row_det.IdSucursal, row_det.IdBodega, row_det.IdCbteVta);
                row_factura.vt_saldo = row_det.SaldoAUX;
                frm_m.set_info(row_factura);
                frm_m.ShowDialog();
                CargarGrillaXCliente();                
            }
            catch (Exception ex)
