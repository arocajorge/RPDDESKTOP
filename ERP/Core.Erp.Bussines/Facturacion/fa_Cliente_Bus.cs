...
namespace Core.Erp.Business.Facturacion
{
    public class fa_Cliente_Bus
    {
        ...
        fa_cliente_contactos_Bus bus_cliente_contaco = new fa_cliente_contactos_Bus();
                    ...
                    if (ds.Rows.Count > 0)
                    {
                        foreach (DataRow row in ds.Rows)
                        {
                            ...
                            if (string.IsNullOrEmpty(row[0].ToString()) == false)
                            {
                                ...
                                info.Persona_Info.pe_nombreCompleto = string.IsNullOrEmpty(info.Persona_Info.pe_razonSocial) == false ? info.Persona_Info.pe_razonSocial : (info.Persona_Info.pe_apellido + " " + info.Persona_Info.pe_nombre);
        ...
        public Boolean ModificarDB(fa_Cliente_Info info, ref string msg)
        {
            try
            {
                ...
                if (respuesta)
                {
                    ...
                    if (respuesta)
                    {
                        foreach (var item in info.listaContactos)
                        {
