...
namespace Core.Erp.Business.Roles
{
  public  class ro_nomina_x_horas_extras_det_Bus
    {
        public ro_nomina_x_horas_extras_det_Bus()
        {
        ...
        public Boolean pu_CalcularHorasExtras(int idEmpresa, decimal idEmpleado, int idNominaTipo, int idNominaTipoLiqui, int idPeriodo, DateTime fechaInicial, DateTime fechaFinal, Boolean acreditaRol)
        {
            try
            {
                ...
                List<ro_nomina_x_horas_extras_det_Info> listado = new List<ro_nomina_x_horas_extras_det_Info>();
                ...
                foreach (ro_horario_planificacion_Info horarioEmpleado in oListRo_horario_planificacion_Info)
                {
                    ...
                    //AGREGA LOS TIEMPOS
                       f1 = f1.Add((TimeSpan)horarioActual.HoraIni);
                       f2 = f2.Add((TimeSpan)horarioActual.HoraFin);
                    ...
                    //AQUI DEBE VERIFICAR SI TIENE VARIOS TURNOS EN EL MISMO DIA - MODALIDAD DE TURNOS DOBLES 
                        ...
                        {
                            fechaMarcacionInicial = fechaMarcacionInicial.Add((TimeSpan)horarioActual.HoraIni);
                            fechaMarcacionFinal = fechaMarcacionFinal.Add((TimeSpan)horarioActual.HoraFin).AddHours(11);
                        }
                        else
                        {
                            fechaMarcacionInicial = fechaMarcacionInicial.Add((TimeSpan)horarioActual.HoraIni);
                            fechaMarcacionFinal = fechaMarcacionFinal.Add((TimeSpan)horarioActual.HoraIni).AddHours(12);
                        }
                    ...
                    foreach (var item in oListRo_marcaciones_x_empleado_Info)
                    {
                        // cambio por tolerancia de horas y minutos Carlos cedeño
                            ...
                            {
                                if (item.es_Hora > horarioActual.HoraIni)
                                {
                                    item.es_Hora = (TimeSpan)horarioActual.HoraIni;
                                }
                    ...
                    if (horaEntrada1 > (TimeSpan)horarioActual.HoraIni)
                    {
                    ...
                    if (dia_semana >= 1 && dia_semana <= 5)    //VERIFICA QUE LOS DIAS DE TRABAJO SON DE LUNES A VIERNES 
                    {
                        ...
                        if (banderaHorario == false)//TURNO DIURNO
                        {
                            ...
                            if (horaSalida2 < (TimeSpan)horarioActual.HoraFin && horaSalida2 > (TimeSpan)horarioActual.HoraIni)
                            {
                            ...
                            if (horaTrabajada > 0)
                            {
                                ...
                                if (horaSalida2 > (TimeSpan)horarioActual.HoraFin && horaSalida2 > horaEntrada1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            horaExtra50 = (horaSalida2 - (TimeSpan)horarioActual.HoraFin).TotalHours; //TOTAL DE HORAS EXTRAS 50%                                    
                                        }
                                    }
                                }
                                else
                                {
                                    ...
                                    if (horaSalida2 < (TimeSpan)horarioActual.HoraIni)
                                    {
                        ...
                        { //***********************************TURNO NOCTURNO*****************************************
                            ...
                            //CALCULA SALIDAS TEMPRANO                 
                            if (horaSalida2 < (TimeSpan)horarioActual.HoraFin && (unDia + horaSalida2) > (TimeSpan)horarioActual.HoraIni)
                            {
                                horaTemprano = ((TimeSpan)horarioActual.HoraFin - horaSalida2).TotalHours;
                            ...
                            if (horaEntrada1 != horaSalida2)
                            {
                                ...
                                if (dia_semana == 6)//SOLO SABADO
                                {
                                    ...
                                    if (horaSalida2 > (TimeSpan)horarioActual.HoraIni && horaSalida2 < finalHora50)
                                    {
                                        ...
                                        if (horaSalida2.TotalHours < (unDia + (TimeSpan)horarioActual.HoraFin).TotalHours)
                                        {
                                            horaTemprano = ((unDia + (TimeSpan)horarioActual.HoraFin) - horaSalida2).TotalHours;
                                        }
    ...
    public Boolean EliminarBD(int IdEmpresa, int IdNomina, int IdNominaTipo, int IdPeriodo)
    {
        try
        {
            return oData.EliminarBD(IdEmpresa, IdNomina, IdNominaTipo, IdPeriodo);
    ...
    public List<ro_nomina_x_horas_extras_det_Info> Get_Info_Nomina_X_Horas_Extras(int idEmpresa, int idNominaTipo, int idNominaTipoLiqui, int idPeriodo, ref string msg)
    {
        try
        {
            return oData.Get_List_Nomina_X_Horas_Extras(idEmpresa, idNominaTipo, idNominaTipoLiqui, idPeriodo, ref msg);
        }
        catch (Exception ex)
        {
            Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
    ...
    public List<ro_nomina_x_horas_extras_det_Info> Get_Info_Nomina_X_Horas_Extras(int idEmpresa)
    {
        try
        {
            return oData.Get_List_Nomina_X_Horas_Extras(idEmpresa);
        }
        catch (Exception ex)
        {
            Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
