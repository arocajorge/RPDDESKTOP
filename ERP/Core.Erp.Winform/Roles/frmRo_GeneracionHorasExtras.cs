...
namespace Core.Erp.Winform.Roles
{
    public partial class frmRo_GeneracionHorasExtras : Form
    {
        ...
        //BUS
            ...
            ro_nomina_x_horas_extras_det_Bus oRo_Nomina_X_Horas_Extras_Bus = new ro_nomina_x_horas_extras_det_Bus();
        ...
        //INFO
            ...
            BindingList<ro_nomina_x_horas_extras_det_Info> oBindingListro_nomina_x_horas_extras_det_Info = new BindingList<ro_nomina_x_horas_extras_det_Info>();
        //DATA
        ...
        private void viewEmpleado_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                var data = viewEmpleado.GetRow(e.RowHandle) as ro_nomina_x_horas_extras_det_Info;
                if (data == null)
                    return;
                              ...
                              if (item.es_AcreditaHorasExtras == true)
                                {
                                    ...
                                    List<ro_nomina_x_horas_extras_det_Info> ListaActualiza = new List<ro_nomina_x_horas_extras_det_Info>();

                                    ListaActualiza = oBindingListro_nomina_x_horas_extras_det_Info.Where(v => v.IdEmpleado == item.IdEmpleado).ToList();
        ...
        void bw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            else
            {
                ...
                oBindingListro_nomina_x_horas_extras_det_Info = new BindingList<ro_nomina_x_horas_extras_det_Info>(oRo_Nomina_X_Horas_Extras_Bus.Get_Info_Nomina_X_Horas_Extras(param.IdEmpresa, _idNominaTipo, _idNominaTipoLiqui, _idPeriodo, ref mensaje));
                gridEmpleado.DataSource = oBindingListro_nomina_x_horas_extras_det_Info;
            ...
            try
            {
                if (checkSeleccionarTodos.Checked == true)
                {
                    if (MessageBox.Show("Esta Seguro que desea seleccionar todos los registro?", "Anulacion", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        foreach (var item in oBindingListro_nomina_x_horas_extras_det_Info)
                        {
        ...
        private void ucRo_PeriodoXNomina_event_cmbPeriodo_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                oBindingListro_nomina_x_horas_extras_det_Info = new BindingList<ro_nomina_x_horas_extras_det_Info>(oRo_Nomina_X_Horas_Extras_Bus.Get_Info_Nomina_X_Horas_Extras(param.IdEmpresa,Convert.ToInt32( cmbnomina.EditValue),Convert.ToInt32( cmbnominaTipo.EditValue),Convert.ToInt32(cmbPeriodos.EditValue), ref mensaje));
                gridEmpleado.DataSource = oBindingListro_nomina_x_horas_extras_det_Info;
                gridEmpleado.RefreshDataSource();

                if (oBindingListro_nomina_x_horas_extras_det_Info.Count() > 0)
                {
        ...
        private void Calcular_total_autorizados()
        {
            try
            {
                ...
                {
                    var data = viewEmpleado.GetRow(i) as ro_nomina_x_horas_extras_det_Info;
