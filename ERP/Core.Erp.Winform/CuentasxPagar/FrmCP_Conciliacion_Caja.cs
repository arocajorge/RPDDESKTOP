    ...
    public partial class frmCP_Conciliacion_Caja : Form
    {
        ...
        private void Cargar_Combos()
        {
            ...
            try
            {
                ...
                cmb_ctaCble.DataSource = List_PlanCta;
                ...
                cp_orden_pago_tipo_Bus bus_op_tipo = new cp_orden_pago_tipo_Bus();
                list_OrdenTipPago = bus_op_tipo.Get_List_orden_pago_tipo_x_Empresa(param.IdEmpresa);
        ...
        public Boolean Valida()
        {
            try
            {
                ...
                if (checkBox_OP.Checked == true)
                {
                    if (this.cmbBeneficiario.Get_Persona_beneficiario_Info() == null || this.cmbBeneficiario.Get_Persona_beneficiario_Info().IdEmpresa == 0)
                    {
                        MessageBox.Show("Ingrese el beneficiario de la orden de pago", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return false;
                    }
                }
        ...
        private void get_Conciliacion_det_Ingr_x_conciliar()
        {
            ...
            {
                Asignar_ingresos();
                Info_Conciliacion.Detalle_x_Ingresos = new List<cp_conciliacion_Caja_det_Ing_Caja_Info>(BindingList_conciliacion_det_x_Ing);
                foreach (var item in Info_Conciliacion.Detalle_x_Ingresos)
                {
                    if (!item.Marcado)
                    {
                        item.valor_disponible = item.Saldo;
                    }
                }
            }
            catch (Exception ex)
        ...
        public Boolean Grabar()
        {
            try
            {
                ...
                if (Bus_ConciCaj_B.GrabarDB(Info_Conciliacion, ref MensajeError))
                {
                    ...
                    if (Info_Conciliacion.IdEstadoCierre == "EST_CIE_CER")
                    {
                        if (MessageBox.Show("Desea imprimir la conciliación de caja?",param.Nombre_sistema,MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            Imprimir();
                        }
                    }
                    ...
                    if (MessageBox.Show("¿Está seguro que desea Eliminar este registro?", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        switch (param.IdCliente_Ven_x_Default)
                        {
                            ...
                            case Cl_Enumeradores.eCliente_Vzen.GRAFINPRENT:
                                ...
                                break;
                            default:
                                if (MessageBox.Show("Para anular la factura presione [SI], para desvincularla de la conciliación presione [NO]", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                                    eliminar_desvincular = true;
                                else
                                    eliminar_desvincular = false;
                                break;
                        ...
                        string mensaje = Bus_ConciCaj_B.Eliminar_facturas_o_vales(info.IdEmpresa_OGiro, info.IdTipoCbte_Ogiro, info.IdCbteCble_Ogiro, "FACTURA", eliminar_desvincular,txt_NConciliacionCaja.Text == "" ? 0 : Convert.ToDecimal(txt_NConciliacionCaja.Text));
                        if (mensaje != "eliminar_linea")
                            MessageBox.Show(mensaje, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        else
                        {
                            MessageBox.Show(eliminar_desvincular == true ? "Registro anulado exitósamente" : "Registro desvinculado exitósamente", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                            gridView_ConciCaja.DeleteSelectedRows();
                        }

                        btn_Recalcular_Click(null, null);
                    }
                }
            }
            catch (Exception ex)
        ...
        void Genera_Diario_OP()
        {
            try
            {
                ...
                Info_Tipo_OP = list_OrdenTipPago.FirstOrDefault(q => q.IdTipo_op == "OTROS_CONC");
                        ...
                        if (info.IdCbteCble_movcaja != 0)
                        {
                            ...
                            switch (param.IdCliente_Ven_x_Default)
                            {
                                ...
                                case Cl_Enumeradores.eCliente_Vzen.GRAFINPRENT:
                                    ...
                                    break;
                                default:
                                    if (MessageBox.Show("Para anular vale presione [SI], para desvincularlo de la conciliación presione [NO]", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                                        eliminar_desvincular = true;
                                    else
                                        eliminar_desvincular = false;
                                    break;
                            ...
                            string mensaje = Bus_ConciCaj_B.Eliminar_facturas_o_vales(info.IdEmpresa_movcaja, info.IdTipocbte_movcaja, info.IdCbteCble_movcaja, "VALE", eliminar_desvincular, txt_NConciliacionCaja.Text == "" ? 0 : Convert.ToDecimal(txt_NConciliacionCaja.Text));
                            if (mensaje != "eliminar_linea")
                                MessageBox.Show(mensaje, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            else
                            {
        ...
        void formaCajaMan_event_FrmCa_Caja_Movimiento_Egreso_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                foreach (var item in BindingList_conciliacion_det_x_ValeCaja.Where(q=>q.IdEmpresa == param.IdEmpresa && q.IdTipocbte_movcaja == List[0].IdTipoCbte && q.IdCbteCble_movcaja == List[0].IdCbteCble))
                {
                    item.cm_valor = List.FirstOrDefault(q => q.dc_Valor > 0).dc_Valor;
                }
                Re_calcular();
            }
            catch (Exception ex)
                ...
                foreach (var item in BindingList_conciliacion_det_x_Ing.OrderBy(q => q.cm_fecha).ToList())
                {
                    ...
                    if (total_egresos > 0)
                    {
                        ...
                        else
                        {
                            ...
                            item.valor_disponible = item.valor_aplicado;
                            item.Marcado = true;
                        }
                    }
                }                
