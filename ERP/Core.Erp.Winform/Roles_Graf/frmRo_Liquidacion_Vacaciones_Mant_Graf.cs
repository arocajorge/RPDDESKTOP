...
{
    public partial class frmRo_Liquidacion_Vacaciones_Mant_Graf : Form
    {
        public frmRo_Liquidacion_Vacaciones_Mant_Graf()
        {
            InitializeComponent();
            CarGarEmpleado();
            Cargar_combo();
        ...
        List<ro_rubro_tipo_Info> lista_rubro_ = new List<ro_rubro_tipo_Info>();
        ro_Nomina_Tipoliqui_Bus bus_nomina_tipo = new ro_Nomina_Tipoliqui_Bus();
        List<ro_Nomina_Tipoliqui_Info> lista_nominatipo = new List<ro_Nomina_Tipoliqui_Info>();
        ro_Empleado_Novedad_Det_Bus bus_novedad = new ro_Empleado_Novedad_Det_Bus();
        BindingList<ro_Empleado_Novedad_Det_Info> lista_novedas = new BindingList<ro_Empleado_Novedad_Det_Info>();
        ro_Historico_Liquidacion_Vacaciones_Det_DescNovedad_Bus bus_detalle_nov = new ro_Historico_Liquidacion_Vacaciones_Det_DescNovedad_Bus();
        ...
        public void Get_Historico_cancelacion()
        {
            try
            {
                ...
                Info_historico_cancelacion.IdSolicitud_Vacaciones = Info_General.IdSolicitud;
                            ...
                            if (detalle.ModificarDB(item,ref id_detalle_OP,ref mensajeError))
                            {
                                ...
                                if (Bus_liquidacion_vacaciones.GrabarBD(Info_historico_cancelacion, ref IdSolicitud))
                                {
                                    ...
                                    if (bandera_sigrabo)
                                    {
                                        ...
                                        {
                                            item_.IdLiquidacion = IdSolicitud;
                                            sec = sec + 1;
                                            if
                                            (sec == 1)                                        
        ...
        public void Set(ro_SolicitudVacaciones_Info info)
        {
            try
            {
                ...
                detalle =new BindingList<ro_Historico_Liquidacion_Vacaciones_Det_Info>( bus_detalle.Get_Lis(info.IdEmpresa,Convert.ToInt32( info.IdEmpleado), info.IdSolicitud));
                ...
                if (detalle.Count() == 0)
                {
                    while (fechaDesde <= info.Anio_Hasta)
                    {
                        ...
                        infod.IdLiquidacion = info.IdSolicitud;
                ...
                {
                    lista_novedas = new BindingList<ro_Empleado_Novedad_Det_Info>(bus_novedad.Get_Novedades_Pendientes(info.IdEmpresa, info.IdEmpleado));
                    gridControl_novedades.DataSource = lista_novedas;
                }
                else
                {
        ...
        private void gridView_Detalle_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                {
                    if (lista_novedas.Where(v => v.check == true).Count() > 0)
                    {
                        descuentos = lista_novedas.Where(v => v.check == true).Sum(v => v.Valor);
                    }
                    else
                    {
                        descuentos = 0;
                    ...
                    if (info_parametro.DescuentaIESS_LiquidacionVacaciones == true)
                    {
                            ...
                            txttotal_cancelar.EditValue = Convert.ToDouble(cancelar) - iess - descuentos;
                            txtiess.EditValue = iess;
        ...
        private void Cargar_combo()
        {
            try
            {
                ...
                lista_nominatipo = bus_nomina_tipo.Get_List_Nomina_Tipoliqui_x_Nomina_Tipo(param.IdEmpresa, 1);
                cmb_proceso.DataSource = lista_nominatipo;
                cmb_proceso.DisplayMember = "DescripcionProcesoNomina";
                cmb_proceso.ValueMember = "IdNomina_TipoLiqui";





                lista_rubro_ = bus_rubro.Get_list_rubros_concepto(param.IdEmpresa);
                cmb_rubro.DataSource = lista_rubro_;
        ...
        private void gridView_novedad_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
