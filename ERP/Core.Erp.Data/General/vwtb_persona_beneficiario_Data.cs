  ...
  public  class vwtb_persona_beneficiario_Data
    {
      ...
      public List<vwtb_persona_beneficiario_Info> Get_List_PersonaBeneficiario(int IdEmpresa, string IdTipo_Persona)
      {
          try
          {
              ...
              List<vwtb_persona_beneficiario_Info> lM = new List<vwtb_persona_beneficiario_Info>();
              EntitiesGeneral OEUs = new EntitiesGeneral();
              lM = (from selec in OEUs.vwtb_persona_beneficiario
                    where selec.IdEmpresa == IdEmpresa
                    && selec.IdTipo_Persona.Contains(IdTipo_Persona)
                    select new vwtb_persona_beneficiario_Info
                    {
                        IdEmpresa = selec.IdEmpresa,
                        IdBeneficiario = selec.IdBeneficiario,
                        IdTipo_Persona = selec.IdTipo_Persona,
                        IdPersona = selec.IdPersona,
                        IdEntidad = selec.IdEntidad,
                        Codigo = selec.Codigo,
                        Nombre = selec.Nombre,
                        pr_girar_cheque_a = selec.pr_girar_cheque_a,
                        pe_razonSocial = selec.pe_razonSocial,
                        pe_cedulaRuc = selec.pe_cedulaRuc,
                        pe_Naturaleza = selec.pe_Naturaleza,
                        IdCtaCble = selec.IdCtaCble,
                        IdCentroCosto = selec.IdCentroCosto,
                        IdSubCentroCosto = selec.IdSubCentroCosto,                        
                        IdCtaCble_Anticipo = selec.IdCtaCble_Anticipo,
                        IdCtaCble_Gasto = selec.IdCtaCble_Gasto,
                        Estado = selec.Estado,
                    }).ToList();

              lM.ForEach(q => q.NombreCompleto = "[" + IdTipo_Persona + "]" + "[" + q.IdEntidad.ToString() + "]" + "[" + q.Nombre + "]");
              return (lM);
          }
          catch (Exception ex)
          {
