    ...
    public partial class FrmIn_Producto_Mant : Form
    {
        ...
        public Boolean Anular()
        {
            try
            {
                ...
                string validacion = Bus_Producto.validar_anulacion(param.IdEmpresa, Info_Producto.IdProducto);

                if (validacion != "OK")
                {
                    MessageBox.Show(validacion,param.Nombre_sistema,MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
                    return false;    
                }
        ...
        private bool grabar_lote_0(decimal IdProducto_padre)
        {
            try
            {
                ...
                if (!(Info_Producto == null))
                {
                    ...
                    Info_Producto.lote_num_lote = "LOTE0";
                    ...
                    Info_Producto.Aparece_modu_Inventario = true;
                    Info_Producto.Aparece_modu_Ventas = true;

                    Info_Producto.pr_descripcion = Info_Producto.pr_descripcion.Trim();
        ...
        void ucGe_Menu_event_btnAnular_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Esta seguro que desea eliminar el Item",param.Nombre_sistema, MessageBoxButtons.YesNo)== System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    {
                        ucGe_Menu.Enabled_bntAnular = false;
                        MessageBox.Show("Anulacion Exitosa",param.Nombre_sistema,MessageBoxButtons.OK,MessageBoxIcon.Asterisk); 
                    } else { }
                }
            }
            catch (Exception ex)
