...
namespace Core.Erp.Winform.Facturacion
{
    public partial class frmFa_Nota_Cred_Mant : Form
    {
        ...
        private void agregar_producto_en_combo(decimal IdProducto)
        {
            try
            {
                if (listProducto.Where(q => q.IdProducto == IdProducto).Count() > 0)
                    return;
                in_Producto_Info i_producto = BusProducto.Get_info_Producto_para_combo(param.IdEmpresa, IdProducto);
                if (i_producto == null)
                    return;
                listProducto.Add(i_producto);
                cmbProducto_grid.DataSource = listProducto;
                gridViewNotaCreDeb.RefreshRow(gridViewNotaCreDeb.FocusedRowHandle);
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
        ...
        private void btn_BuscarFactura_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IdCliente != 0)
                {
                    ...
                    {
                        Info_Doc_rel.Valor_Aplicado = Info_Doc_rel.saldo == null ? 0 : (double)Info_Doc_rel.saldo;
